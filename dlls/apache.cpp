...
#define SF_APACHE_TURRETONLY	1024 
...
class CApache : public CBaseMonster
{
	...
	int	 ObjectCaps( void ) { return CBaseMonster::ObjectCaps() | FCAP_FORCE_TRANSITION; }
...
void CApache :: Spawn( void )
{
	...
	pev->effects		|= EF_BRIGHTLIGHT;	// this should do the trick i guess?
...
void CApache :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	STOP_SOUND( ENT(pev), CHAN_STATIC, "apache/ap_rotor1.wav" );
...
void CApache :: HuntThink( void )
{
	...
	if ( FBitSet ( pev->spawnflags, SF_APACHE_TURRETONLY ) ) 
		return;
...
void CApache :: Flight( void )
{
	...
	if (m_iSoundState == 0)
	{
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor1.wav", 1.0, 0.25, 0, 110 );
		 EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_whine1.wav", 0.5, 0.4, 0, 110 );
	...
	else
	{
		...
		if (pPlayer)
		{
			...
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, "apache/ap_rotor1.wav", 1.0, 0.3, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
