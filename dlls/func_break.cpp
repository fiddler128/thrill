...
void CBreakable::Spawn( void )
{
	...
	if ( FBitSet( pev->spawnflags, SF_BREAK_NOTSOLID ) )
		pev->solid = SOLID_NOT;
	else
		pev->solid		= SOLID_BSP;
...
void CBreakable::Precache( void )
{
	...
	case matGlass:
		...
		m_usShards = PRECACHE_EVENT( 1, "events/shards.sc" );
		break;
	case matMetal:
...
//		return;
	...
	case matMetal:
		...
		rgpsz[3] = "debris/metal1.wav";
		rgpsz[4] = "debris/metal3.wav";
		rgpsz[5] = "debris/metal2.wav";
		i = 6;
		break;
...
void CBreakable::Die( void )
{
	...
	case matGlass:
		...
		case 2:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "debris/bustglass3.wav", fvol, ATTN_NORM, 0, pitch);	
			break;
		case 3:	EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "Common/glass.wav", fvol, ATTN_NORM, 0, pitch);	
			break;
		}
//		cFlag = BREAK_GLASS;
//		cFlag = BREAK_2;
//		cFlag = BREAK_SMOKE;
	...
	else 
	{
		...
		vecOrigin = VecBModelOrigin(pev);

		float size;

		if (pev->size.x >= pev->size.y) 
		{
			if (pev->size.x >= pev->size.z)
				size = pev->size.x;
			else
				size = pev->size.z;
		}
		...
		{
			if (pev->size.y >= pev->size.z)
				size = pev->size.y;
			else
				size = pev->size.z;
		}

		PLAYBACK_EVENT_FULL( FEV_GLOBAL, edict(), m_usShards, 0.0, (float*)&vecOrigin, (float*)&g_vecZero, size, 0.0, entindex(), 0, 0, 0);
	}
...
void CPushable :: Move( CBaseEntity *pOther, int push )
{
	...
	if ( CVAR_GET_FLOAT("sv_pushablefix") > 0 )
	{
