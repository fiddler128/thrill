...
/*	MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
	...
	*/
	...
	m_flNextFireball = gpGlobals->time + 0.15;
...
void CRpgRocket :: FollowThink( void  )
{
	...
	if ( m_flNextFireball <= gpGlobals->time )
	{
	...
	if ( !pev->waterlevel )
	m_flNextFireball = gpGlobals->time + 0.05;
	else
	m_flNextFireball = gpGlobals->time + 0.3;
	}
...
void CRpg::Spawn( )
{
	...
	if ( pev->sequence == 0 )
	{
		UTIL_SetSize(pev, Vector(-16, -16, -12), Vector(16, 16, 16));
	}
}
...
TYPEDESCRIPTION	CRpgRocket::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CRpgRocket, m_flNextFireball, FIELD_TIME ),
};
