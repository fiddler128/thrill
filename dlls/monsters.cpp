...
int CBaseMonster::Restore( CRestore &restore )
{
	...
	if ( m_pGoalEnt && FClassnameIs( m_pGoalEnt->pev, "path_corner") )
	{
...
// Debug, Draw the triangulation
		if ( CVAR_GET_FLOAT("sv_drawroute") >= 2 )
		{
			MESSAGE_BEGIN( MSG_BROADCAST, SVC_TEMPENTITY );
				WRITE_BYTE( TE_SHOWLINE );
				WRITE_COORD( vecStart.x );
...
void CBaseMonster :: SetEyePosition ( void )
{
	...
	if ( vecEyePosition == g_vecZero )
	{
	//	pev->view_ofs = Center();
		ALERT ( at_aiconsole, "%s has no view_ofs!\n", STRING ( pev->classname ) );
	}
	else pev->view_ofs = vecEyePosition;
}
...
BOOL CBaseMonster :: FCheckAITrigger ( void )
{
	...
	case AITRIGGER_DEATH:
		...
		{
			ALERT ( at_console, "CBaseMonster: Death trigger condition!!\n" );
			fFireTarget = TRUE;
		}
		break;
...
/*
void CBaseMonster :: SUB_SetDefaultThink( void )
{
	ALERT ( at_console, "MonsterThink was set by SUB_SetDefaultThink!\n" );
	SetThink ( &CBaseMonster::CallMonsterThink ); 
}
*/
