...
void CHAssassin :: Shoot ( void )
{
	...
	if ( m_flLastShot + 1.5 > gpGlobals->time )
	{
		return;
	}
	...
	CCrossbowBolt *pBolt = CCrossbowBolt::BoltCreate(CROSSBOW_BOLT_POISONED);
	pBolt->pev->origin = GetGunPosition();
...
{
	if ( !HasConditions( bits_COND_ENEMY_OCCLUDED ) && flDist > 64 && flDist <= 2048 && m_flLastShot + 1.5 < gpGlobals->time /* && flDot >= 0.5 */ /* && NoFriendlyFire() */ )
	{
//		ALERT ( at_console, "LastShot is: %.2f Time is %.2f \n", m_flLastShot, gpGlobals->time);
		TraceResult	tr;
		...
		{
//			ALERT ( at_console, "Can fire! LastShot is: %.2f Time is %.2f \n", m_flLastShot, gpGlobals->time);
			return TRUE;
		}
	}
	return FALSE;
